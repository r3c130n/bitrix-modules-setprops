    ...
    foreach ($_POST['FIELDS'] as $ID => $arFields) {
        ...
        if ($bWorkFlow) {
            ...
            $bCanWrite = CIBlockDocument::CanUserOperateDocument(
                            CBPCanUserOperateOperation::WriteDocument, $USER->GetID(), $ID, array(
                        "IBlockId" => $IBLOCK_ID,
        ...
        if (array_key_exists("PREVIEW_PICTURE", $arFields)) {
            $arFields["PREVIEW_PICTURE"] = CIBlock::makeFileArray(
                            $arFields["PREVIEW_PICTURE"], $_REQUEST["FIELDS_del"][$ID]["PREVIEW_PICTURE"] === "Y", $_REQUEST["FIELDS_descr"][$ID]["PREVIEW_PICTURE"]
            );
        }

        if (array_key_exists("DETAIL_PICTURE", $arFields)) {
            $arFields["DETAIL_PICTURE"] = CIBlock::makeFileArray(
                            $arFields["DETAIL_PICTURE"], $_REQUEST["FIELDS_del"][$ID]["DETAIL_PICTURE"] === "Y", $_REQUEST["FIELDS_descr"][$ID]["DETAIL_PICTURE"]
            );
        }
        ...
        foreach ($arFields as $k => $v) {
            if (
                    substr($k, 0, strlen("PROPERTY_")) == "PROPERTY_" && $k != "PROPERTY_VALUES"
            ) {
                ...
                if (isset($arFileProps[$prop_id])) {
                    foreach ($v as $prop_value_id => $file) {
                        $v[$prop_value_id] = CIBlock::makeFilePropArray(
                                        $v[$prop_value_id], $_REQUEST["FIELDS_del"][$ID][$k][$prop_value_id]["VALUE"] === "Y", $_REQUEST["FIELDS_descr"][$ID][$k][$prop_value_id]
                        );
                    }
                }
                ...
                if (isset($_REQUEST["FIELDS_descr"][$ID][$k]) && is_array($_REQUEST["FIELDS_descr"][$ID][$k])) {
                    foreach ($_REQUEST["FIELDS_descr"][$ID][$k] as $PROPERTY_VALUE_ID => $ar) {
                        if (
                                is_array($ar) && isset($ar["VALUE"]) && isset($v[$PROPERTY_VALUE_ID]["VALUE"]) && is_array($v[$PROPERTY_VALUE_ID]["VALUE"])
                        )
        ...
        foreach ($arIBlock["FIELDS"] as $FIELD_ID => $field) {
            if (
                    $field["IS_REQUIRED"] === "Y" && !array_key_exists($FIELD_ID, $arFields) && $FIELD_ID !== "DETAIL_PICTURE" && $FIELD_ID !== "PREVIEW_PICTURE"
            )
    ...
    if ($bCatalog) {
        if ($boolCatalogPrice && (isset($_POST["CATALOG_PRICE"]) || isset($_POST["CATALOG_CURRENCY"]))) {
            ...
            $rsCatalogGroups = CCatalogGroup::GetList(
                            array('BASE' => 'DESC'), array("LID" => LANGUAGE_ID), false, false, array('ID', 'BASE')
            );
            ...
            foreach ($CATALOG_PRICE as $elID => $arPrice) {
                if (
                        !(CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $elID, "element_edit") && CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $elID, "element_edit_price"))
                )
                    continue; 
                $bError = false;
                ...
                if ($strSaveWithoutPrice != 'Y') {
                    if (isset($arPrice[$arBaseGroup['ID']])) {
                        ...
                        $arBasePrice = CPrice::GetBasePrice(
                                        $elID, $CATALOG_QUANTITY_FROM[$elID][$arBaseGroup['ID']], $CATALOG_QUANTITY_FROM[$elID][$arBaseGroup['ID']], false
                        );
                ...
                if (!empty($arCatalogGroupList)) {
                    foreach ($arCatalogGroupList as &$arCatalogGroup) {
                        ...
                        if ($arPrice[$arCatalogGroup["ID"]] != doubleval($CATALOG_PRICE_old[$elID][$arCatalogGroup["ID"]]) || $arCurrency[$arCatalogGroup["ID"]] != $CATALOG_CURRENCY_old[$elID][$arCatalogGroup["ID"]]) {
                            if ('Y' == $arCatalogGroup["BASE"]) { // if base price check extra for other prices
                                ...
                                $db_res = CPrice::GetListEx(
                                                array(), $arPrFilter, false, false, array("ID", "PRODUCT_ID", "CATALOG_GROUP_ID", "PRICE", "CURRENCY", "QUANTITY_FROM", "QUANTITY_TO", "EXTRA_ID")
                                );
            ...
            case "wf_status":
                if ($bWorkFlow) {
                    ...
                    if (
                        ...
                        if (CIBlockElement::WF_GetStatusPermission($new_status) > 0 || CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $ID, "element_edit_any_wf_status")) {
...
if ($wf_status_id) {
    $rsData = CIBlockElement::GetList(
                    $arOrder, $arFilter, false, false, $arSelectedFields
    );
        ...
        if ($wf_status_id !== false) {
            ...
            if ($LAST_ID != $arElement['ID']) {
                $rsData2 = CIBlockElement::GetList(
                            ...
                            "ID" => $LAST_ID,
                            "SHOW_HISTORY" => "Y"
                                ), false, array("nTopCount" => 1), array("ID", "WF_STATUS_ID")
                );
                if ($arRes = $rsData2->Fetch()) {
    ...
    $rsData = CIBlockElement::GetList(
                    $arOrder, $arFilter, false, $arNavParams, $arSelectedFields
    );
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    if ($bWorkFlow) {
        ...
        if ($LAST_ID != $arRes['ID']) {
            $rsData2 = CIBlockElement::GetList(
                        ...
                        "ID" => $LAST_ID,
                        "SHOW_HISTORY" => "Y"
                            ), false, array("nTopCount" => 1), $arSelectedFields
            );
            if (isset($arCatGroup)) {
                $arRes_tmp = array();
    ...
    foreach ($arSelectedProps as $aProp) {
        ...
        foreach ($arProperties[$aProp["ID"]] as $prop_id => $prop) {
            ...
            if (array_key_exists("GetAdminListViewHTML", $arUserType)) {
                $arViewHTML[] = call_user_func_array($arUserType["GetAdminListViewHTML"], array(
                    $prop,
                    array(
                        "VALUE" => $prop["~VALUE"],
            ...
            if ($bUserMultiple) {
                ...
                $arEditHTML[] = call_user_func_array($arUserType["GetPropertyFieldHtml"], array(
                    $prop,
                    array(
                        "VALUE" => $prop["VALUE"],
                        "DESCRIPTION" => $prop["DESCRIPTION"],
        ...
        if ($aProp["MULTIPLE"] == "Y") {
            ...
            if (
                    $prop["PROPERTY_TYPE"] !== "G" && $prop["PROPERTY_TYPE"] !== "L" && $prop["PROPERTY_TYPE"] !== "F" && !$bUserMultiple
            )
    ...
    if ($bBizproc) {
        $arDocumentStates = CBPDocument::GetDocumentStates(
                        array("iblock", "CIBlockDocument", "iblock_" . $IBLOCK_ID), array("iblock", "CIBlockDocument", $f_ID)
        );
        ...
        foreach ($arDocumentStates as $kk => $vv) {
            $canViewWorkflow = CIBlockDocument::CanUserOperateDocument(
                            CBPCanUserOperateOperation::ViewWorkflow, $USER->GetID(), $f_ID, array("AllUserGroups" => $arRes["CURRENT_USER_GROUPS"], "DocumentStates" => $arDocumentStates, "WorkflowId" => $kk)
            );
            if (!$canViewWorkflow)
                continue;
...
if ($bCatalog && !empty($arRows)) {
    ...
    if (!empty($arProductGroupIDs)) {
        $rsSets = CCatalogProductSet::getList(
                        array(), array('OWNER_ID' => array_keys($arProductGroupIDs), 'SET_ID' => 0, 'TYPE' => CCatalogProductSet::TYPE_GROUP), false, false, array('ID', 'OWNER_ID', 'ITEM_ID', 'SET_ID', 'TYPE')
        );
    ...
    if (isset($arSelectedFieldsMap['CATALOG_MEASURE']) && !empty($arMeasureIDs)) {
        $rsMeasures = CCatalogMeasure::getList(
                        array(), array('@ID' => array_keys($arMeasureIDs)), false, false, array()
        );
    ...
    if (isset($arSelectedFieldsMap['CATALOG_MEASURE_RATIO'])) {
        $rsRatios = CCatalogMeasureRatio::getList(
                        array(), array('@PRODUCT_ID' => $arRowKeys), false, false, array('ID', 'PRODUCT_ID', 'RATIO')
        );
...
foreach ($arRows as $f_ID => $row) { 
    ...
    if ($bWorkFlow) {
        ...
        if ($row->arRes['orig']['LOCK_STATUS'] == "red") {
            ...
            if (
                    isset($arElementOps[$f_ID]) && isset($arElementOps[$f_ID]["element_edit"]) && (2 <= $STATUS_PERMISSION)
            ) {
            ...
            if (
                    $boolIBlockElementAdd && (2 <= $STATUS_PERMISSION)
            ) {
                $arActions[] = array(
                    "ICON" => "copy",
            ...
            if (strlen($row->arRes['DETAIL_PAGE_URL']) > 0) {
                ...
                if (
                    ...
                    if (isset($arElementOps[$f_ID]) && isset($arElementOps[$f_ID]["element_edit"]) && isset($arElementOps[$f_ID]["element_edit_any_wf_status"])) {
                        $arActions[] = array("SEPARATOR" => true);
                        $arActions[] = array(
                            "ICON" => "view",
            ...
            if ($row->arRes["WF_STATUS_ID"] > 1 && isset($arElementOps[$f_ID]) && isset($arElementOps[$f_ID]["element_edit"]) && isset($arElementOps[$f_ID]["element_edit_any_wf_status"])
            ) {
                $arActions[] = array(
                    "ICON" => "edit_orig",
            ...
            if (
                    isset($arElementOps[$f_ID]) && isset($arElementOps[$f_ID]["element_delete"]) && (2 <= $STATUS_PERMISSION)
            ) {
    ...
    elseif ($bBizproc) {
        $bWritePermission = CIBlockDocument::CanUserOperateDocument(
                        CBPCanUserOperateOperation::WriteDocument, $USER->GetID(), $f_ID, array(
                    "IBlockId" => $IBLOCK_ID,
        ...
        $bStartWorkflowPermission = CIBlockDocument::CanUserOperateDocument(
                        CBPCanUserOperateOperation::StartWorkflow, $USER->GetID(), $f_ID, array(
                    "IBlockId" => $IBLOCK_ID,
                    "AllUserGroups" => $row->arRes["CURRENT_USER_GROUPS"],
                    "DocumentStates" => $arDocumentStates,
                        )
        ...
        if (
                ...
                isset($arElementOps[$f_ID]) && isset($arElementOps[$f_ID]["element_bizproc_start"])
                )
        ) {
            $arActions[] = array(
                "ICON" => "",
        ...
        if (
                isset($arElementOps[$f_ID]) && isset($arElementOps[$f_ID]["element_edit"])
        ) {
            $arActions[] = array(
                "ICON" => "edit",
                "TEXT" => GetMessage("IBEL_A_CHANGE"),
...
<?
echo FindUserID(
        "find_el_modified_user_id", $find_el_modified_user_id, "", "find_form", "5", "", " ... ", "", ""
);
?>
        </td>
    </tr>
...
<?
echo FindUserID(
        "find_el_created_user_id", $find_el_created_user_id, "", "find_form", "5", "", " ... ", "", ""
);
?>
        </td>
    </tr>
